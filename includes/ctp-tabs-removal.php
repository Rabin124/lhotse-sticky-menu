...
if ( ! function_exists( 'ctp_switch' ) ) {
	...
	function ctp_switch() {
		...
		if ( ! check_ajax_referer( 'ctp_tabs_nonce', 'security', false ) ) {
			wp_die( esc_html__( 'Invalid Nonce', 'lhotse-sticky-menu' ) );
		} else {
			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( esc_html__( 'Permission denied!', 'lhotse-sticky-menu' ) );
			}
			...
			if ( update_option( 'ctp_options', $option_value ) ) {
				...
				esc_html_e( 'Connection Error. Please try again.', 'lhotse-sticky-menu' );
			}
		}
